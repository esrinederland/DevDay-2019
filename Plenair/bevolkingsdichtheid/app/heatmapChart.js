define(["require", "exports", "esri/views/layers/support/FeatureFilter", "esri/Color", "./constants"], function (require, exports, FeatureFilter, Color, constants_1) {
    ...
    var layerView = null;
    var data = [];
    var start = new Color([255,0,0]);//"#FF420E");//[255,111,97]);//"#eaf5ff");
    var end = new Color([0,255,0]);//"#89DA59");//[144,192,72]);//"#003E72");
    var numCols = 3;
    var numRows = 4;
    ...
    function updateGrid(newData, lv, clearHighlight) {
        ...
        for (var _a = 0, data_2 = data; _a < data_2.length; _a++) {
            ...
            ctx.strokeText("" + Math.round(value), col * cellWidth + cellWidth / 2, row * cellHeight + cellHeight / 2);
            ctx.fillText("" + Math.round(value), col * cellWidth + cellWidth / 2, row * cellHeight + cellHeight / 2);
        }
    ...
    function onCellSelect(cell) {
        var season = constants_1.seizoenen[cell.row];
        var dichtheid = constants_1.dichtheden[cell.col];
        if (mousemoveEnabled) {
            highlighted = { col: cell.col, row: cell.row };
            layerView.filter = new FeatureFilter({
                where: "Q = '" + season + "' AND JAAR = '" + dichtheid + "'"
            });
        }
        updateGrid();
    }
